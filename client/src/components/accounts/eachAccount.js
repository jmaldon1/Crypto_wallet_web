    ...
    async getBalances(id){
        try{
            ...
            if(results.addresses.filter(account => account.used === false && account.change === false).length === 0){
                await this.getNewAddress(id, false)
            }else{
             ...
             <div className={this.state.curAccountData.defaultAccount === true ? "tab-pane fade show active" : "tab-pane fade show"} id={"v-pills-" + this.state.curAccountData.id} role="tabpanel" aria-labelledby={"v-pills-" + this.state.curAccountData.id + "-tab"}>
                <nav>
                  <div className="nav nav-tabs" id="nav-tab" role="tablist">
                    <a className="nav-item nav-link" onClick={() => { this.tabClick(this.state.curAccountData) }} id={"nav-transactions-tab-id-" + this.state.curAccountData.id} data-toggle="tab" href={"#nav-transactions-id-" + this.state.curAccountData.id} role="tab" aria-controls={"#nav-transactions-id-" + this.state.curAccountData.id} aria-selected="false">
                    <i className="fa fa-exchange" aria-hidden="true"></i>
                    Transactions <span className="badge badge-info">{this.state.newTxNotification}</span></a>
                    <a className="nav-item nav-link" id={"nav-send-tab-id-" + this.state.curAccountData.id} data-toggle="tab" href={"#nav-send-id-" + this.state.curAccountData.id} role="tab" aria-controls={"#nav-send-id-" + this.state.curAccountData.id} aria-selected="false"><i className="fa fa-arrow-circle-up" aria-hidden="true"></i>
                    Send</a>
                    <a className="nav-item nav-link active" id={"nav-receive-tab-id-" + this.state.curAccountData.id} data-toggle="tab" href={"#nav-receive-id-" + this.state.curAccountData.id} role="tab" aria-controls={"#nav-receive-id-" + this.state.curAccountData.id} aria-selected="true"><i className="fa fa-arrow-circle-down" aria-hidden="true"></i>
                    Receive</a>
                ...
                <div className="tab-content" id="nav-tabContent">
                    <div className="tab-pane fade" id={"nav-transactions-id-" + this.state.curAccountData.id} role="tabpanel" aria-labelledby={"nav-transactions-tab-id-" + this.state.curAccountData.id}>
                        <TxHistory accountData={this.state.curAccountData} getBalances={this.getBalances} newTx={this.newTx} resetNewTxCount={this.state.resetTxNotification} resetFeedback={this.resetFeedback} /> 
                    </div>
